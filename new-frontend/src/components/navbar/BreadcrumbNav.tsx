...
export default function BreadcrumbNav() {
  ...
  if (firstSegment === 'tours') {
    // /tours or /tours/[id]
    breadcrumbs.push({ label: 'Tours', href: '/tours', isCurrent: segments.length === 1 });
    ...
    breadcrumbs.push({ label: 'Tournaments', href: '/tournaments', isCurrent: segments.length === 1 });
      ...
      {breadcrumbs.map((item, index) => (
        <div key={index} className="flex items-center gap-2">
          ...
          {item.href ? (
            ...
            <span
              className="text-gp-dark font-medium"
              aria-current="page"
            >
              {item.label}
            </span>
