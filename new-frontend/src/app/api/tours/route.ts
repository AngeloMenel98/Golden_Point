...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to fetch tours' }, { status: response.status });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!name || typeof name !== 'string' || name.trim() === '') {
      return NextResponse.json({ success: false, error: 'El nombre del tour es requerido' }, { status: 400 });
    }

    if (!clubsId || !Array.isArray(clubsId) || clubsId.length === 0) {
      return NextResponse.json({ success: false, error: 'Selecciona al menos un club' }, { status: 400 });
    }
    ...
    const response = await fetch(`${API_URL}/tour/create`, {
      ...
      credentials: 'include',
      body: JSON.stringify({ tourTitle: name.trim(), clubsId }),
    });

    const data = await response.json();

    if (!response.ok) {
      const errorMessage = data.error?.message || data.message || 'Error al crear el tour';
