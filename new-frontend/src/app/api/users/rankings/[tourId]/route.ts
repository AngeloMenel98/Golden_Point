...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/api/users/rankings/${tourId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to fetch tournament rankings' }, { status: response.status });
    }
