...
export async function GET(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to fetch tournament' }, { status: response.status });
    }
    ...
    const tournament = {
      ...
      masterScore: parseInt(String(t.masterscore || t.master_score || t.masterScore || 0), 10),
...
export async function DELETE(
  ...
  try {
    ...
    if (!response.ok) {
      const errorMessage = data.error?.message || data.message || 'Error al eliminar el torneo';
