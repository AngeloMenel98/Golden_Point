...
async function fetchTournament(id: string, token: string): Promise<Tournament | null> {
  try {
    const response = await fetch(`${API_URL}/api/tournament/${id}`, {
      method: 'GET',
      headers: {
    ...
    return {
      ...
      categories: Array.isArray(t.categories) ? t.categories as Category[] : [],
  ...
  if (!tournament) {
    return (
      <div className="text-center p-12">
        <h1 className="text-2xl font-bold text-gp-dark mb-4">Torneo no encontrado</h1>
        <p className="text-gp-gray">El torneo que buscas no existe o no tienes acceso.</p>
      </div>
    );
  }
